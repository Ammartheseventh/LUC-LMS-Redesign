...
function AcademicStatusCard() {
  return (
    <div className="bg-white rounded-lg border border-slate-200/90 shadow-sm p-5 card-transition flex flex-col justify-between gap-2">
      ...
      <ul className="divide-y divide-slate-100 text-xs">
        {academicStatusLinks.map((link, index) => (
          <li 
            className="py-2.5 px-1 flex items-center justify-between group text-sm font-medium text-slate-700 hover:bg-slate-50 hover:text-brand-400 cursor-pointer transition"
            key={index}
          >
